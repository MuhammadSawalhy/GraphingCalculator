...
{
    public sealed class Acos : Node
    {
        public override int ArgumentNumber => 1;
        public override SyntaxType Syntax_Type => SyntaxType.Function;
        public override string Type => "Acos";
        public Acos(Node children) : base(children)
        {
        }
        public override double Calculate(CalculationSetting CalculationSetting, Dictionary<Loyc.Symbol, Node> tempVars)
        {
            switch (CalculationSetting.AngleType)
            {
                case Main.AngleType.Degree:
                    return Math.Acos(Children[0].Calculate(CalculationSetting, tempVars)) * 180 / Math.PI;
                case Main.AngleType.Grads:
                    return Math.Acos(Children[0].Calculate(CalculationSetting, tempVars)) * 200 / Math.PI;
                default:
                    return Math.Acos(Children[0].Calculate(CalculationSetting, tempVars));
            }
        }
