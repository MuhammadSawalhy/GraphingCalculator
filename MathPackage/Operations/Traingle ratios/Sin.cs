...
namespace MathPackage.Operations
{
    public sealed class Sin : Node
    {
        ...
        public Sin(Node children) : base(children)
        {
        }
        public override double Calculate(CalculationSetting CalculationSetting, Dictionary<Loyc.Symbol, Node> tempVars)
        {
            switch (CalculationSetting.AngleType)
            {
                case Main.AngleType.Degree:
                    return Math.Sin(Children[0].Calculate(CalculationSetting, tempVars) / 180 * Math.PI);
                case Main.AngleType.Grads:
                    return Math.Sin(Children[0].Calculate(CalculationSetting, tempVars) / 200 * Math.PI);
                default:
                    return Math.Sin(Children[0].Calculate(CalculationSetting, tempVars));
            }
        }
