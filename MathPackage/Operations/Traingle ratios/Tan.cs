...
namespace MathPackage.Operations
{
    public sealed class Tan : Node
    {
        ...
        public Tan(Node children) : base(children)
        {
        }
        public override double Calculate(CalculationSetting CalculationSetting, Dictionary<Loyc.Symbol, Node> tempVars)
        {
            switch (CalculationSetting.AngleType)
            {
                case Main.AngleType.Degree:
                    return Math.Tan(Children[0].Calculate(CalculationSetting, tempVars) / 180 * Math.PI);
                case Main.AngleType.Grads:
                    return Math.Tan(Children[0].Calculate(CalculationSetting, tempVars) / 200 * Math.PI);
                default:
                    return Math.Tan(Children[0].Calculate(CalculationSetting, tempVars));
            }
        }
